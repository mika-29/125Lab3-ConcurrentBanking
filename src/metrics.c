...
void print_metrics(Transaction *transactions, int num_transactions) {
    ...
    for (int i = 0; i < num_transacs; i++) {
        Transaction *transac = &transacs[i];

        const char *status_str;
        switch (transac->status) {
            case TX_RUNNING: 
                status_str = "RUNNING"; 
                break;
    ...
    double avg_wait   = num_txns > 0 ? (double)total_wait / num_txns : 0.0;
    double throughput = max_end_tick > 0
                        ? (double)num_txns / max_end_tick
                        : 0.0;
